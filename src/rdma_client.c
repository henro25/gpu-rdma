...
/* Track the current data size we want to transfer (in bytes). */
static size_t xfer_size = 0;
 ...
 * First, calls `rdma_buffer_register` to register a memory region for the server metadata.
 * Then, fills up a sge with the server metadata memory region's data, 
 * Finally, posts the receive work request.
 */
...
int main(int argc, char **argv) {
    ...
    /* A quick array of sizes: 1MB, 2MB, 4MB, 8MB, 16MB, 32MB, 64MB, 128MB,
       256MB, 512MB, 1024MB (1GB). Expand if needed. */
    const size_t test_sizes[] = {
        1UL << 20,  2UL << 20,  4UL << 20,  8UL << 20,
       16UL << 20, 32UL << 20, 64UL << 20, 128UL << 20,
      256UL << 20, 512UL << 20, 1024UL << 20
    };
    ...
    /* Parse only '-a' and '-p' now. We removed '-s'. */
    while ((option = getopt(argc, argv, "a:p:")) != -1) {
        switch (option) {
            case 'a':
                ret = get_addr(optarg, (struct sockaddr*) &server_sockaddr);
                if (ret) {
                    rdma_error("Invalid IP address\n");
                    return ret;
                }
                break;
            case 'p':
    ...
    for(int i = 0; i < num_tests; i++) {
        ...
        double elapsed_sec, throughput_mbps;

        xfer_size = test_sizes[i];  // use global xfer_size
        ...
        if (ret) {
            rdma_error("Failed to prepare connection, ret = %d\n", ret);
            goto cleanup_loop;
        }
        ...
        if (ret) {
            rdma_error("Failed to pre-post recv, ret = %d\n", ret);
            goto cleanup_loop;
        }
        ...
        if (ret) {
            rdma_error("Failed to exchange metadata, ret = %d\n", ret);
            goto cleanup_loop;
        }

        /* Measure RDMA write + read time */
        clock_gettime(CLOCK_MONOTONIC, &start);
        ret = client_remote_memory_ops();
        clock_gettime(CLOCK_MONOTONIC, &end);

        if (ret) {
            rdma_error("Remote memory ops failed, ret = %d\n", ret);
            goto cleanup_loop;
        }

        /* Calculate elapsed time and throughput (MB/s) */
        elapsed_sec = (double)(end.tv_sec - start.tv_sec)
                    + (double)(end.tv_nsec - start.tv_nsec) / 1e9;
        throughput_mbps = (double)xfer_size / (1024.0 * 1024.0) / elapsed_sec;

        /* Check data correctness */
        if (check_src_dst()) {
            printf("Data mismatch for size %zu\n", xfer_size);
        } else {
            printf("Data matches for size %zu\n", xfer_size);
        }

        /* Print throughput results */
        printf("Time = %.6f sec, Throughput = %.2f MB/s\n",
               elapsed_sec, throughput_mbps);

    cleanup_loop:
        /* Cleanly disconnect and free resources */
...
// 	struct sockaddr_in server_sockaddr;
// 	int ret, option;
// 	bzero(&server_sockaddr, sizeof server_sockaddr); // Clear the server_sockaddr structure
// 	server_sockaddr.sin_family = AF_INET; // Set the address family to IPv4
// 	server_sockaddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK); // Set the IP address to loopback
// 	/* buffers are NULL */
// 	src = dst = NULL; 
// 	/* Parse Command Line Arguments */
// 	while ((option = getopt(argc, argv, "s:a:p:")) != -1) {
// 		switch (option) {
// 			// case 's': // String to be transferred
