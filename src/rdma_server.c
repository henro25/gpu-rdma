 ...
 */
 ...
 static struct ibv_comp_channel *io_completion_channel = NULL;  // just for post work requests
 ...
 /*
  * In this function, we assume that cm_client_id is already set to a valid identifier (asserted elsewhere).
  * Here, we prepare the client connection by pre-posting a receive buffer, which is used to receive 
  * the client's RDMA credentials.
  *
  * The following resources are created:
  *
  * 1. Protection Domain (PD):
  *    - Created via ibv_alloc_pd().
  *    - The Protection Domain serves as a safety boundary, grouping together all allocated RDMA resources 
  *      (like memory regions and queue pairs) to ensure that only resources from the same PD interact with each other.
  *
  * 2. Completion Channel:
  *    - Created via ibv_create_comp_channel().
  *    - This channel is used for handling asynchronous I/O completion notifications from the RDMA device.
  *      It allows the application to wait for and process I/O events separate from connection management events.
  *
  * 3. Completion Queue (CQ):
  *    - Created via ibv_create_cq().
  *    - The Completion Queue is where the RDMA subsystem places completion metadata (e.g., work completions)
  *      for processing, enabling the application to check the status of previous I/O operations.
  *
  * 4. Queue Pair (QP):
  *    - Created via rdma_create_qp().
  *    - The Queue Pair consists of a pair of send and receive queues that facilitate the actual data transfer 
  *      between the RDMA client and server, acting as the main communication channel.
  */
 ...
 {
     printf("Usage:\n");
     printf("rdma_server: [-a <server_addr>] [-p <server_port>] [-n <num_connections>]\n");
 ...
 int main(int argc, char **argv) 
 {
     ...
     server_sockaddr.sin_addr.s_addr = htonl(INADDR_ANY); /* Defaults to listening on all interfaces */
     ...
     while ((option = getopt(argc, argv, "a:p:")) != -1) {
         switch (option) {
             ...
             case 'p':
                 ...
                 break;
             case 'n':
                 NUM_CONNECTIONS = strtol(optarg, NULL, 0);
                 break;
             default:
                 usage();
                 break;
     ...
     for (int i = 0; i < NUM_CONNECTIONS; i++) {
         ...
         ret = process_rdma_cm_event(cm_event_channel, 
             ...
             &cm_event);
         if (ret) {
             rdma_error("Failed to get connect request event, ret = %d \n", ret);
             return ret;
         }
         cm_client_id = cm_event->id;
         ret = rdma_ack_cm_event(cm_event);
         if (ret) {
             rdma_error("Failed to acknowledge connect request event, ret = %d \n", ret);
             return ret;
         }
         debug("Connect request event received from client %d\n with id %p\n", i, cm_client_id);
     ...
     ret = rdma_destroy_id(cm_server_id);
     if (ret) {
         rdma_error("Failed to destroy server id cleanly, ret = %d \n", ret);
         return ret;
     }
     rdma_destroy_event_channel(cm_event_channel);
     printf("Server shut-down is complete \n");
